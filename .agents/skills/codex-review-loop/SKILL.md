...
1. Run the review script with `run_in_background=true`:
   ...
   ```
   Note: Codex CLI v0.105.0+ does not support combining `--base`,
   `--commit`, or `--uncommitted` with a custom prompt. The built-in review
   logic is used automatically when any diff target is provided.
   For `--uncommitted` mode:
   ```
   bash <skill-dir>/scripts/codex-subagent.sh --uncommitted
   ```
